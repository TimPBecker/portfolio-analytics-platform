...
# -----------------------------------------------------------------------------
# 3. Sidebar: Database Status, Selection & Application Metadata & Tabs Execution
# -----------------------------------------------------------------------------
def run_dashboard():
    """Main Streamlit application entrypoint and layout renderer."""
    with st.sidebar:
        ...
        # Active DB Engine (with safe fallback if missing credentials)
        try:
            active_engine = get_engine(database=selected_db)
        except Exception:
            from portfolio_core.db import get_test_engine
            active_engine = get_test_engine(fallback_to_sqlite=True)
